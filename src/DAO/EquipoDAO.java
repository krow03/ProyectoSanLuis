...
public class EquipoDAO implements PatronDAO<EquipoDTO>{
	...
	private static final String SQL_INSERT="INSERT INTO Equipos (idEquipos,Aulas_idAulas,nombre,discoDuro,ram,disponibleHD,ipEquipo) VALUES (?,?,?,?,?,?,?)";
	...
	public boolean insertar(EquipoDTO t) throws SQLException {
		try {
			...
			ps.setInt(6, t.getDisponibleHDD());
	...
	public boolean actualizar(EquipoDTO t) {
		...
		try {
			...
			ps.setInt(5, t.getDisponibleHDD());
			ps.setString(6, t.getIpEquipo());
	...
	public EquipoDTO buscar(Object pk) {
		...
		try {
			...
			if (rs.next()==true){
				equip = new EquipoDTO(rs.getInt("idEquipos"),rs.getString("ipEquipo"),rs.getString("nombre"),rs.getInt("discoDuro"),rs.getInt("ram"),rs.getInt("disponibleHD"),rs.getInt("Aulas_idAulas"));
				return equip;
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public ArrayList<EquipoDTO> listarTodos() {
		...
		try {
			...
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
				EquipoDTO equip = new EquipoDTO(rs.getInt("idEquipos"),rs.getString("ipEquipo"),rs.getString("nombre"),rs.getInt("discoDuro"),rs.getInt("ram"),rs.getInt("disponibleHD"),rs.getInt("Aulas_idAulas"));
				lista.add(equip);
			}
			rs.close();
		} catch (SQLException e) {
